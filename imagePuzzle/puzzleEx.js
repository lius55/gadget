	...
	return this.filter( ":has( img )" ).each(
		...
		function( intI ){
			...
			function InitPuzzle(){
				...
				for (var intRowIndex = 0 ; intRowIndex < intRows ; intRowIndex++){
					...
					for (var intColIndex = 0 ; intColIndex < intColumns ; intColIndex++){
						...
						var index = intRowIndex * intColumns + intColIndex;
						jPiece = $( "<div style='font-size:20px;color:#FFFFFF;font-weight:bold;'>"+ index +"</div>" );
				...
				// Change check flag after shuffle
				checkGame = true;
				...
				var desAry = new Array(0,1,2,3,4,5,6,7,8);
				for(var row = 0; row < intRows; row++){
					for(var col = 0; col < intColumns; col++){
						orgAry.push(parseInt(arr2DBoard[ row ][ col ].attr("index")));
					}
				}
				
				getSolution(orgAry, desAry);
			...
			function PieceClickHandler( objEvent ){
				...
				// Nothing happens when game is over
			...
			// Check to see if the game has over
			function CheckGame(){
				var successFlag = true;
				for(var row = 0; row < intRows; row++){
					for(var col = 0; col < intColumns; col++){
						if(arr2DBoard[ row ][ col ].attr("index") != row * intColumns + col){
							successFlag = false;
							break;
						}
					}
				...
				if(successFlag){
					...
					arr2DBoard[ intRows - 1 ][ intColumns - 1 ].animate(
						{
							opacity: 1
						},
						800,
			...
			// Clear the text of index
			function clearIndex(){
				for(var row = 0; row < intRows; row++){
					for(var col = 0; col < intColumns; col++){
						arr2DBoard[ row ][ col ].html("<div><br/></div>");
					}
				}
				//You got it,Man~
				alert("Hurray!"); 
