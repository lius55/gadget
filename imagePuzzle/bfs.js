...
var rootAry = new Array(1,2,3,4,5,6,7,8,0); 	//初始状态
var xAxis = 3; 									//列数
var yAxis = 3; 									//行数
var aryLength = xAxis * yAxis; 					//方格数目
var desAry = new Array(1,2,3,4,0,6,7,5,8); 	//目标状态
...
var whiteValue = 8;                             //空白节点值
var rootNode;									//根节点
var checkNode; 									//当前查看节点

function getSolution(pRootAry, pDesAry){
	
	rootAry = pRootAry;
	desAry = pDesAry;
	rootNode = new Node(nodeIndex,rootAry,0,null);	
	checkNode = rootNode;
	...
	if(notFound){
		...
		for(var i = resultList.length - 1; i >= 0; i--){
			if(i < resultList.length - 1){
				resultStr += resultList[i+1].ary[resultList[i].move[0]] + ", Move ";
				resultStr += outPutDirection(resultList[i].move[1]) + "<br/>"; 
			}
			resultStr += resultList[i].ary.toString();
			if(i > 0){ resultStr +="<br/>"; }
		}
